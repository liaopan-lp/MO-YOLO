...
import argparse
from MOTR.models.structures import Boxes, matched_boxlist_iou, pairwise_iou
import numpy as np
from MOTR.util.misc import inverse_sigmoid
from MOTR.util.box_ops import box_cxcywh_to_xyxy
from MOTR.models.ops.modules import MSDeformAttn

def motr_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default=r'E:\data\MOT17\test\MOT17-01-DPM\img1')
    ...
    parser.add_argument('--resume', default=r'E:\cv\MOTR\model_motr_final.pth', help='resume from checkpoint')
    parser.add_argument('--start_epoch', default=0, type=int, metavar='N',
                        help='start epoch')
    parser.add_argument('--eval', action='store_true')
    parser.add_argument('--vis', default=True,action='store_true')
    parser.add_argument('--num_workers', default=2, type=int)
    parser.add_argument('--pretrained', default=r'E:\cv\MOTR\model_motr_final.pth', help='resume from checkpoint')
...
class MOTRDeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        encoder_layer = MOTRDeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                              dropout, activation,
                                                              num_feature_levels, nhead, enc_n_points,
                                                              sigmoid_attn=sigmoid_attn)
        self.encoder = DeformableTransformerEncoder(encoder_layer, num_encoder_layers)

        decoder_layer = MOTRDeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                              dropout, activation,
                                                              num_feature_levels, nhead, dec_n_points, decoder_self_cross,
                                                              sigmoid_attn=sigmoid_attn, extra_track_attn=extra_track_attn)
        self.decoder = MOTRDeformableTransformerDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec)
...
class MOTRDeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
...
class MOTRDeformableTransformerDecoderLayer(nn.Module):
...
class MOTRDeformableTransformerDecoder(nn.Module):
...
def build_deforamble_transformer(args):
    return MOTRDeformableTransformer(
