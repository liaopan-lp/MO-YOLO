from ultralytics import YOLO, DecoderTracker, RTDETR
import os
if __name__ == '__main__':
    ...
    # model = DecoderTracker("yolo_track.yaml")  # 7443199 7447807 7443199
    # model = DecoderTracker("yolov8m_track.yaml")
    # model = DecoderTracker(r"E:\cv\KITTI_detection\weights\best.pt")  # 加载预训练模型（建议用于训练）
    # model = DecoderTracker(r"E:\cv\MOT17_detection\best.pt")  # 加载预训练模型（建议用于训练）
    ...
    #     model = DecoderTracker(r"F:\cv\tracking_pretrain\runs\track\dancetrack\weights\best.pt")  # 加载预训练模型（建议用于训练）
    #
    #     model(folder_path+'/'+subpath+r'/img1')

    # subfolders 列表中包含了指定目录下的所有子文件夹的名称

    # model = DecoderTracker(r"C:\Users\a\Desktop\train3\weights\best.pt")  # 加载预训练模型（建议用于训练）
    # model = DecoderTracker(r"F:\cv\tracking_pretrain\runs\track\MOT17\weights\best.pt")  # 加载预训练模型（建议用于训练）
    # model = DecoderTracker(r"F:\cv\tracking_pretrain\runs\track\train2\weights\best.pt")  # 加载预训练模型（建议用于训练）
    ...
    #     model = DecoderTracker(r'F:\cv\ultralytics_trackingv3\runs\track\train5\weights\best.pt')  # 加载预训练模型（建议用于训练）
    ...
    KITTI_url = [1]
    for i in KITTI_url:
        model = DecoderTracker(r"F:\cv\ultralytics_trackingv3\runs\track\train6\weights\best.pt")  # 加载预训练模型（建议用于训练）
