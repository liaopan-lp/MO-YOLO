        ...
        if boxes:
            for c in boxes.cls.unique():
                ...
                try:
                    ...
                    return log_string if probs is not None else f'{log_string}(no tracking), '
        return log_string
...
class TrackResults(Results):
    def __init__(self, orig_img, path, names, boxes=None, track_id=None, masks=None, probs=None,
                 keypoints=None) -> None:
        """Initialize the Results class."""
        super().__init__(orig_img, path, names, boxes, masks, probs, keypoints)
    ...
    def plot(
        ...
        if pred_boxes and show_boxes:
            i = 0
            track_ids = reversed(track_ids)
            # print(track_ids)
            for d in reversed(pred_boxes):
                c, conf, id = int(d.cls), float(d.conf) if conf else None, None if d.id is None else int(
                    d.id.item())
                id = track_ids[i]
                name = ('' if id is None else f'id:{id} ') + names[c]
                label = (f'{name} {conf:.2f}' if conf else name) if labels else None
    ...
    def save_txt(self, txt_file, save_conf=False):
        ...
        if probs is not None:
            ...
            for j, d in enumerate(boxes):
                ...
                line = (track_id, c, *d.xywhn.view(-1))
                if masks:
                    seg = masks[j].xyn[0].copy().reshape(-1)  # reversed mask.xyn, (n,2) to (n*2)
                    line = (c, *seg)
                if kpts is not None:
                    kpt = torch.cat((kpts[j].xyn, kpts[j].conf[..., None]), 2) if kpts[j].has_visible else kpts[
                        j].xyn
                    line += (*kpt.reshape(-1).tolist(),)
                line += (conf,) * save_conf + (() if id is None else (id,))
                texts.append(('%g ' * len(line)).rstrip() % line)
