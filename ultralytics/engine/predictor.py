...
class BasePredictor:
    ...
    def get_save_dir(self):
        if self.args.task == 'track':
            self.args.project = 'runs'
        project = str(self.args.project) or Path(SETTINGS['runs_dir']) + "/" + self.args.task
    ...
    def write_track_results(self, idx, results, batch):
        """Write inference results to a file or directory."""
        p, im, _ = batch
        log_string = ''
        if len(im.shape) == 3:
        ...
        self.txt_path = str(self.save_dir / 'labels' / p.stem) + ('' if self.dataset.mode == 'image' else f'_{frame}')
        import os
        if not os.path.exists(self.save_dir / 'labels') and (self.args.save or self.args.save_txt):
            os.mkdir(self.save_dir / 'labels')
            ...
            for i in range(n):
                ...
                if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                    if (self.args.task != 'track'):
                        ...
                        s += self.write_track_results(i, self.results, (p, im, im0))
    ...
    def setup_model(self, model, verbose=True):
        ...
        # model_2_params_file = r'C:\Users\a\Documents\test\motr_predictor7.txt'
        # # # 将模型参数保存到两个不同的文本文件
