...
def get_cdn_group(batch,
    ...
    try:
        ...
        dn_cls_embed = dn_cls_embed.to(padding_cls.dtype)  # 将dn_cls_embed转换为与padding_cls相同的数据类型
        padding_cls[(dn_b_idx, map_indices)] = dn_cls_embed
...
def get_track_cdn_group(batch,
    ...
    tgt_size = num_dn + num_track_queries + num_queries
    attn_mask = torch.zeros([tgt_size, tgt_size], dtype=torch.bool)
    # match query cannot see the reconstruct
    attn_mask[num_dn:, :num_dn] = True
    # reconstruct cannot see each other.
    ...
    dn_meta = {
        'dn_pos_idx': [p.reshape(-1) for p in pos_idx.cpu().split(list(gt_groups), dim=1)],
        'dn_num_group': num_group,
        'dn_num_split': [num_dn, num_track_queries + num_queries]}
