...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, pred_bboxes, pred_scores, gt_bboxes, gt_cls, gt_groups, masks=None, gt_mask=None):
        ...
        nan_indices = torch.isnan(C)

        # 使用 torch.where 函数将NaN值替换为0
        C = torch.where(nan_indices, torch.tensor(0.0), C)
...
from MOTR.util.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
from scipy.optimize import linear_sum_assignment
    ...
    def forward(self, pred_bboxes, pred_scores, gt_bboxes, gt_cls, gt_groups, masks=None, gt_mask=None):
        ...
        Params:
            ...
            targets: This is a list of targets (len(targets) = batch_size), where each target is a dict containing:
                 "track_id": Tensor of dim [num_target_boxes] (where num_target_boxes is the number of ground-truth
        ...
        with torch.no_grad():
            ...
            cost_bbox = (pred_bboxes.unsqueeze(1) - gt_bboxes.unsqueeze(0)).abs().sum(-1)  # (bs*num_queries, num_gt)
            # Compute the GIoU cost between boxes, (bs*num_queries, num_gt)
            cost_giou = 1.0 - bbox_iou(pred_bboxes.unsqueeze(1), gt_bboxes.unsqueeze(0), xywh=True, GIoU=True).squeeze(
                -1)
