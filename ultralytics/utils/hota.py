...
class HOTA(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(data['gt_ids'], data['tracker_ids'])):
            ...
            try:
                # print(potential_matches_count.shape)
                # print(gt_ids_t)
                # print(tracker_ids_t)
                potential_matches_count[gt_ids_t[:, np.newaxis], tracker_ids_t[np.newaxis, :]] += sim_iou

            except:
                potential_matches_count[0:len(gt_ids_t), 0:len(tracker_ids_t)] += sim_iou

            # Calculate the total number of dets for each gt_id and tracker_id.

            if len(gt_id_count) <= max(gt_ids_t):
                gt_ids_t -= 1
            try:
            ...
            try:
                if len(tracker_id_count) <= max(tracker_ids_t):
                    tracker_ids_t -= min(list(tracker_ids_t))
            except:
                pass
            #
            # print(tracker_id_count.shape)
            # print(tracker_id_count)
            # print(tracker_ids_t.shape)
            # print(tracker_ids_t)
            tracker_ids_t -= 1
            try:
                tracker_id_count[tracker_ids_t] += 1
            except:
                tracker_id_count = tracker_id_count[0]
                tracker_id_count[tracker_ids_t] += 1
        ...
        for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(data['gt_ids'], data['tracker_ids'])):
            ...
            similarity = data['similarity_scores'][t]
            # score_mat = np.squeeze(global_alignment_score[gt_ids_t[:, np.newaxis], 0:len(tracker_ids_t)],
            #                        axis=1) * similarity
            score_mat = np.squeeze(global_alignment_score[gt_ids_t[:, np.newaxis], 0:len(tracker_ids_t)]
                                   ) * similarity
            ...
            try:
                ...
                score_mat = np.squeeze(score_mat)
                try:
                    match_rows, match_cols = linear_sum_assignment(-score_mat)
                except:
                    score_mat = score_mat[0, :, :]
                    match_rows, match_cols = linear_sum_assignment(-score_mat)
